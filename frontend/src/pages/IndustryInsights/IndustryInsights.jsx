...
const trendColor = t =>
  ["up", "easier"].includes(t) ? "var(--green)" :
  ["down", "harder"].includes(t) ? "var(--red)" :
  "var(--amber)"

const scoreColor = s =>
  s >= 8 ? "var(--green)" : s >= 6 ? "var(--teal)" : s >= 4 ? "var(--amber)" : "var(--red)"

// ── Shared sub-components (defined OUTSIDE main component) ───
// This is critical — defining these outside prevents remounting on every render
...
const Spin = ({ size = 16 }) => (
  <span
    style={{
      ...
      borderTopColor: "#fff",
      borderRadius: "50%",
      animation: "ins-spin .75s linear infinite",
...
const ErrorBanner = ({ msg, onRetry }) => (
  <div className="error-banner">
    ...
    {onRetry && (
      <button
        onClick={onRetry}
        style={{ marginLeft: "auto", background: "none", border: "none", color: "var(--red)", cursor: "pointer", fontSize: 13, fontFamily: "var(--font)", fontWeight: 700, textDecoration: "underline" }}
      >
        Retry
      </button>
    )}
...
// ── MarketSelector — outside component to avoid remount ──────
  ...
  <div style={{ display: "flex", gap: 7, flexWrap: "wrap" }}>
    {MARKETS.map(m => (
      <button
        ...
        className="mkt-btn"
        style={{
          background: market === m.id ? "rgba(108,92,231,0.14)" : "rgba(255,255,255,0.03)",
          borderColor: market === m.id ? "var(--accent)" : "var(--border2)",
          color: market === m.id ? "var(--accent2)" : "var(--subtle)",
        }}
...
// ── SearchBar — outside component to avoid remount on every keystroke ──
// Previously defined inside IndustryInsights, which caused the input to
// lose focus after every character typed. Moved here to fix that bug.
const SearchBar = ({ searchRole, setSearchRole, market, setMarket, isLoading, onSearch, onChipClick }) => (
  <div style={{ display: "flex", flexDirection: "column", gap: 12, marginBottom: 24 }}>
    <div style={{ display: "flex", gap: 10 }}>
      ...
      <button className="ins-btn" onClick={onSearch} disabled={isLoading || !searchRole.trim()}>
    ...
    <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
      {SUGGESTED_ROLES.slice(0, 12).map(r => (
        <button key={r} className="suggestion-chip" onClick={() => onChipClick(r)}>{r}</button>
      ))}
    </div>
  </div>
)
...
export default function IndustryInsights() {
  ...
  const [errPulse, setErrPulse]         = useState(null)
  const [forecast, setForecast]             = useState(null)
  const [loadingForecast, setLoadingForecast] = useState(false)
  const [errForecast, setErrForecast]         = useState(null)

  // Data tabs
  const [salaryData, setSalaryData]           = useState(null)
  const [loadingSalary, setLoadingSalary]     = useState(false)
  const [errSalary, setErrSalary]             = useState(null)
  ...
  const [errEmployers, setErrEmployers]           = useState(null)
  ...
  const [errRole, setErrRole]       = useState(null)
          ...
          "employers": [
            {
              ...
              "interview_format": "brief format description",
              "difficulty": "Easy|Medium|Hard|Very Hard",
              "culture_note": "1 sentence about culture",
  ...
  const fetchCalendar = async (role) => {
    ...
    try {
      const data = await callAI(
        `Generate a hiring calendar for "${role}" in ${mktObj.label} (2025). Return JSON:
        {
          "role": "${role}",
          "market": "${mktObj.label}",
          "peak_season": "e.g. September–November for campus",
          "calendar_note": "2 sentences about the hiring cycle",
          "timeline": [
            { "phase": "Phase name", "timing": "Month range", "action": "what to do during this phase" }
          ],
          "market_quirks": "any unusual hiring patterns",
          "networking_tip": "networking advice for this role"
        }
  ...
  const generateEmail = async () => {
    ...
    try {
      const data = await callAI(
        ...
        Make it concise, personalized, confident but not pushy.`
      )
      setEmailResult(data)
    ...
    try {
      const data = await callAI(
        `Generate a realistic career progression path for someone aiming to become a "${pathRole}". Return JSON:
        {
          ...
          "common_mistake": "most common mistake people make",
          ...
          <motion.div key={activeTab} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} transition={{ duration: 0.18 }}>
            ...
            {activeTab === "salary" && (
              <div>
                ...
                <SearchBar
                  ...
                  isLoading={isLoading} onSearch={() => runSearch()} onChipClick={handleChipClick}
                />
                {errSalary && <ErrorBanner msg={errSalary} />}
                {loadingSalary && <LoadingBlock label="Fetching salary benchmarks…" />}
                {!salaryData && !loadingSalary && !errSalary && <EmptyState icon="💰" text="Search any role" sub="Works for any job title across 6 markets" />}
                {salaryData && (
            ...
            {activeTab === "employers" && (
              <div>
                ...
                {!employersData && !loadingEmployers && !errEmployers && <EmptyState icon="🏢" text="Search any role" sub="Find out which companies are actively hiring and how to prepare" />}
                {employersData && (
                    ...
                    {employersData.employers?.map((e, i) => (
                      <motion.div key={i} className="ins-card" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.06 }}>
                        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", flexWrap: "wrap", gap: 10, marginBottom: 12 }}>
                          ...
                          <div style={{ padding: "5px 13px", background: `${diffColor(e.difficulty)}10`, border: `1.5px solid ${diffColor(e.difficulty)}30`, borderRadius: 22, fontSize: 13, fontWeight: 700, color: diffColor(e.difficulty) }}>{e.difficulty}</div>
                        </div>
            ...
            {activeTab === "path" && (
              <div>
                ...
                <p className="sec-sub">Your step-by-step roadmap to any dream role</p>
                ...
                {!pathData && !loadingPath && !errPath && <EmptyState icon="🗺️" text="Where do you want to be?" sub="Enter your dream role and get a realistic roadmap with timelines and milestones" />}
                {pathData && (
                  <motion.div style={{ display: "flex", flexDirection: "column", gap: 14 }} initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }}>
                    <div className="ins-card" style={{ background: "rgba(108,92,231,0.06)", borderColor: "rgba(108,92,231,0.22)" }}>
                    ...
                    {pathData.alternative_paths?.length > 0 && (
                      <div className="ins-card">
                        ...
                        <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                          {pathData.alternative_paths.map((p, i) => <span key={i} style={{ fontSize: 13, padding: "5px 14px", background: "rgba(108,92,231,0.08)", border: "1px solid rgba(108,92,231,0.2)", borderRadius: 22, color: "var(--accent2)" }}>{p}</span>)}
                        </div>
                      </div>
                    )}
                  </motion.div>
              ...
              <div>
                ...
                <div style={{ display: "flex", gap: 10, marginBottom: 16 }}>
                  <div style={{ flex: 1, position: "relative" }}>
                    ...
                    <input className="ins-inp" placeholder="e.g. Equity Research Analyst, DevOps Engineer…"
                  ...
                  <motion.div style={{ display: "flex", flexDirection: "column", gap: 14 }} initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }}>
                    <div className="ins-card" style={{ background: "rgba(108,92,231,0.06)", borderColor: "rgba(108,92,231,0.22)" }}>
                      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 10, marginBottom: 12 }}>
                        ...
                        <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
                          <span style={{ fontSize: 12, padding: "4px 12px", background: "rgba(108,92,231,0.12)", border: "1px solid rgba(108,92,231,0.25)", borderRadius: 20, color: "var(--accent2)", fontWeight: 700 }}>{roleData.difficulty_to_get} to break in</span>
                    ...
                    <div className="ins-card">
                      ...
                      <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                        {roleData.best_companies?.map((c, i) => <span key={i} style={{ fontSize: 13, padding: "5px 14px", background: "rgba(162,155,254,0.08)", border: "1px solid rgba(162,155,254,0.2)", borderRadius: 22, color: "var(--accent2)" }}>{c}</span>)}
                      </div>
              ...
              <div>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 14, marginBottom: 20 }}>
                  <div>
                    ...
                    <textarea className="ins-inp ins-ta ins-inp-plain" placeholder="e.g. Final-year engineering student with internship at Infosys…"
            ...
            {activeTab === "compare" && (
              <div>
                ...
                <div style={{ marginBottom: 14 }}>
                  ...
                  <div style={{ display: "flex", gap: 10 }}>
                    <input className="ins-inp ins-inp-plain" style={{ flex: 1 }} placeholder="e.g. Investment Banking Analyst…"
