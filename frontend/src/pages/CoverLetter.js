...
import {
  Sparkles, ArrowLeft, Copy, Download, RefreshCw,
...
const GLOBAL_CSS = `
  ...
  * { box-sizing: border-box; }
  ...
  /* ── Force dark background + bright text on all inputs ── */
  .cl-inp {
    width: 100%;
    padding: 11px 13px;
  ...
  .cl-inp:focus {
    border-color: #7c6ffa !important;
    box-shadow: 0 0 0 3px rgba(124,111,250,0.15);
  }
  .cl-inp::placeholder {
  ...
  .cl-inp:-webkit-autofill,
  .cl-inp:-webkit-autofill:hover,
  .cl-inp:-webkit-autofill:focus {
    -webkit-box-shadow: 0 0 0px 1000px #1a1d2e inset !important;
    -webkit-text-fill-color: #ffffff !important;
    caret-color: #ffffff;
    border: 1.5px solid #2e3450 !important;
  }

  /* Canva-style paragraph editing */
  .cl-para {
    display: block;
    outline: none;
    border-radius: 4px;
  ...
  .cl-para:hover { background: rgba(124,111,250,0.07); }
  .cl-para:focus { background: rgba(124,111,250,0.12); box-shadow: 0 0 0 2px rgba(124,111,250,0.3); }
...
const SHEET_STYLES = {
  modern: {
    wrap:   { fontFamily: "Georgia, serif", padding: "52px 56px", background: "#fff", color: "#1a1a2e" },
    header: { borderBottom: "3px solid #7c6ffa", paddingBottom: 18, marginBottom: 26 },
    name:   { fontSize: 22, fontWeight: 700, color: "#7c6ffa", letterSpacing: -0.5 },
    contact:{ fontSize: 12.5, color: "#555", marginTop: 5 },
    body:   { fontSize: 14.5, lineHeight: 2, color: "#2a2a3e" },
    date:   { fontSize: 13, color: "#888", marginBottom: 18 },
    addr:   { fontSize: 13.5, color: "#333", marginBottom: 22 },
  },
  classic: {
    wrap:   { fontFamily: "Times New Roman, serif", padding: "56px 60px", background: "#fffef8", color: "#1a1a1a" },
    header: { borderBottom: "1px solid #333", paddingBottom: 14, marginBottom: 28 },
    name:   { fontSize: 20, fontWeight: 700, color: "#111", letterSpacing: 0.5, textTransform: "uppercase" },
    contact:{ fontSize: 12, color: "#555", marginTop: 5 },
    body:   { fontSize: 14, lineHeight: 1.95, color: "#222" },
    date:   { fontSize: 13, color: "#888", marginBottom: 16 },
    addr:   { fontSize: 13, color: "#333", marginBottom: 20 },
  },
  minimal: {
    wrap:   { fontFamily: "'DM Sans', sans-serif", padding: "60px 64px", background: "#fafafa", color: "#222" },
    header: { marginBottom: 34 },
    name:   { fontSize: 18, fontWeight: 600, color: "#555", letterSpacing: 1, textTransform: "uppercase" },
    contact:{ fontSize: 12, color: "#777", marginTop: 6 },
    body:   { fontSize: 14, lineHeight: 2.1, color: "#333" },
    date:   { fontSize: 12.5, color: "#999", marginBottom: 16 },
    addr:   { fontSize: 13, color: "#444", marginBottom: 22 },
  },
  executive: {
    wrap:   { fontFamily: "Garamond, Georgia, serif", padding: "60px 64px", background: "#fff", color: "#1a1010", borderTop: "6px solid #c9a84c" },
    header: { marginBottom: 30 },
    name:   { fontSize: 24, fontWeight: 700, color: "#c9a84c", letterSpacing: -0.3, fontStyle: "italic" },
    contact:{ fontSize: 12.5, color: "#666", marginTop: 6 },
    body:   { fontSize: 15, lineHeight: 2, color: "#2a2010" },
    date:   { fontSize: 13, color: "#999", marginBottom: 18 },
    addr:   { fontSize: 13.5, color: "#333", marginBottom: 22 },
  },
  startup: {
    wrap:   { fontFamily: "'DM Sans', sans-serif", padding: "48px 52px", background: "#0f172a", color: "#e2e8f0" },
    header: { borderBottom: "1px solid #334155", paddingBottom: 16, marginBottom: 26 },
    name:   { fontSize: 20, fontWeight: 700, color: "#38bdf8", letterSpacing: -0.3 },
    contact:{ fontSize: 12.5, color: "#94a3b8", marginTop: 5 },
    body:   { fontSize: 14, lineHeight: 2, color: "#cbd5e1" },
    date:   { fontSize: 13, color: "#64748b", marginBottom: 16 },
    addr:   { fontSize: 13, color: "#94a3b8", marginBottom: 20 },
  },
}
...
const Field = ({ label, hint, icon: Icon, children }) => (
  <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
    <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
      ...
      <label style={{ fontSize: 11.5, fontWeight: 700, textTransform: "uppercase", letterSpacing: 0.9, color: "#6b7280", fontFamily: "'DM Sans',sans-serif" }}>
        {label}
      </label>
    </div>
...
// ── Canva-style editable paragraph ───────────────────────────
...
export default function CoverLetter() {
  useEffect(() => {
    if (!document.getElementById("cl-global-css")) {
      ...
      <header style={S.topbar}>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          {hasResult && <>
            ...
            <motion.button style={{ ...S.topBtn, ...S.topBtnAccent }} onClick={downloadPDF}
      ...
      <div style={S.layout}>
        ...
        <aside style={S.left}>
          ...
          <div style={S.fields}>
            ...
            {activeTab === "basics" && <>
              ...
              <Field label="Full Name" icon={User}>
                <input className="cl-inp" placeholder="e.g. Divit Shah" value={fullName} onChange={e => setFullName(e.target.value)} />
              </Field>
              <Field label="Email" icon={Mail}>
                <input className="cl-inp" type="email" placeholder="you@email.com" value={email} onChange={e => setEmail(e.target.value)} />
              </Field>
              <Field label="Phone">
                <input className="cl-inp" placeholder="+91 98765 43210" value={phone} onChange={e => setPhone(e.target.value)} />
              </Field>
              <Field label="LinkedIn / Portfolio" icon={Globe}>
                <input className="cl-inp" placeholder="linkedin.com/in/yourname" value={linkedin} onChange={e => setLinkedin(e.target.value)} />
              </Field>
              <div style={{ ...S.sh, marginTop: 6 }}>The Role</div>
              <Field label="Job Role *" icon={Briefcase} hint="Required — be specific">
                <input className="cl-inp" placeholder="e.g. Investment Banking Analyst" value={jobRole} onChange={e => setJobRole(e.target.value)} />
              </Field>
              <Field label="Company Name" icon={Building2}>
                <input className="cl-inp" placeholder="e.g. Goldman Sachs" value={companyName} onChange={e => setCompanyName(e.target.value)} />
              </Field>
              <Field label="Industry *" hint="Required">
                <input className="cl-inp" placeholder="e.g. Finance, Tech, Healthcare" value={industry} onChange={e => setIndustry(e.target.value)} />
              </Field>
              <Field label="Hiring Manager Name" icon={User} hint="Personalises the salutation">
                <input className="cl-inp" placeholder="Ms. Priya Mehta (optional)" value={hiringManager} onChange={e => setHiringManager(e.target.value)} />
              </Field>
            </>}
            ...
            {activeTab === "details" && <>
              ...
              <Field label="Your Background" icon={User} hint="2–4 sentences about your experience & strengths">
                <textarea className="cl-inp cl-ta" rows={5}
                  placeholder="CFA Level I candidate with 2 years of equity research..."
              ...
              <Field label="Key Achievement" hint="One result with numbers — makes you memorable">
                <textarea className="cl-inp cl-ta" rows={3}
                  placeholder="Built a DCF model that identified 30% undervaluation — led to ₹12Cr acquisition..."
                  value={keyAchievement} onChange={e => setKeyAchievement(e.target.value)} />
              </Field>
              <Field label="Job Description" icon={AlignLeft} hint="Paste the full JD for maximum personalisation">
                <textarea className="cl-inp cl-ta" rows={7}
            ...
            {activeTab === "style" && <>
              ...
              {hasResult && (
                <div style={{ marginTop: 10, padding: "11px 13px", background: "rgba(52,213,200,0.05)", border: "1px solid rgba(52,213,200,0.15)", borderRadius: 10 }}>
                  <p style={{ fontSize: 12.5, color: "#34d5c8", margin: 0, fontFamily: "'DM Sans',sans-serif" }}>✦ Style changes update the letter design instantly — no regeneration needed.</p>
                </div>
              )}
            </>}
          ...
          <div style={S.genWrap}>
            ...
            <motion.button style={{ ...S.btnGen, opacity: loading ? 0.7 : 1 }}
              onClick={generate} disabled={loading}
              whileHover={!loading ? { scale: 1.02 } : {}} whileTap={{ scale: 0.97 }}>
              {loading
                ? <><span style={S.spinner} />Generating…</>
                : <><Wand2 size={16} />{hasResult ? "Regenerate" : "Generate Cover Letter"}</>}
            </motion.button>
        ...
        <section style={S.right}>
          ...
          {!hasResult ? (
            ...
            <motion.div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 14 }}
              ...
              <div style={S.sheetWrap}>
                <div ref={sheetRef} style={st.wrap}>
                  ...
                  {/* Body — each paragraph independently editable */}
                  <div style={st.body}>
